...
if __name__ == "__main__":
    bond_lengths = np.linspace(0.3, 3.5, 45)
    energies = []
    params = None
    ...
    for r in bond_lengths:
        ...
        data = pyfock_hf_quantities(symbols, coordinates, basis_name="sto-3g", verbose=False)
        energy, params = vqe_energy(data, params=params, max_iter=80, conv_tol=1e-6, verbose=True)
    ...
    plt.xlabel("H-H bond length (A)")
    plt.ylabel("Energy (Hartree)")
    plt.title("H2 dissociation curve from PyFock integrals + PennyLane VQE")
    plt.legend()
    plt.tight_layout()
    # plt.savefig("ex36_H2_VQE_dissociation_curve.png", dpi=200)
    plt.show()
