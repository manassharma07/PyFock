...
# This example shows how to calculate the Overlap matrix using the basis set object created.
# One can specify exactly which elements of the Overlap matrix they want to calculate.
# So, one can either calculate a single element or a continuous block of the matrix using the slice.

#Let's calculate the complete Overlap matrix
print('\n\n\n')
print('Integrals')
...
ovlp = Integrals.overlap_mat_symm(basis)
print(ovlp) 

#Instead of calculating the complete KE matrix you can also just calculate a specific element or a continuous block of the Overlap matrix
#This is done by specifying an additional argument that contains the start and indices of the block of the Overlap matrix 
...
ovlp_subset = Integrals.overlap_mat_symm(basis, slice=[start_row, end_row, start_col, end_col])
print(ovlp_subset) 

# Compare with the slice of the original full matrix
print('\n\nCompare with the slice of the original full matrix')
print('abs(ovlp[start_row:end_row, start_col:end_col] - Vkin_subset).max()')
print(abs(ovlp_subset - ovlp[start_row:end_row, start_col:end_col]).max())  
...
print(Integrals.overlap_mat_symm(basisBig))
