try:
    import cupy as cp
    from cupy import fuse
except Exception as e:
    cp = None
    def fuse(kernel_name):
        def decorator(func):
            return func
        return decorator
import numpy as np
from pyfock.XC.lda_c_pw_mod import lda_c_pw_mod_, lda_c_pw_mod_cupy_
from numba import njit


@njit(cache=True, fastmath=True, error_model="numpy", nogil=True, inline='always')
def gga_c_pbe_sol_(rho, sigma):
    """
    PBEsol correlation functional (spin-unpolarized).
    Same as PBE correlation but with beta = 0.046 instead of 0.06672455060314922.
    Corresponds to GGA_C_PBE_SOL with ID 133 in LibXC.
    Reference: Phys. Rev. Lett. 100, 136406 (2008).
    """
    rho = np.maximum(rho, 1e-12)

    beta = 0.046  # PBEsol value
    gamma = (1 - np.log(2.0)) / np.pi**2

    pi34 = (3 / (4 * np.pi)) ** (1 / 3)
    rs = pi34 * rho ** (-1 / 3)
    norm_dn = np.sqrt(sigma)
    ec, vc = lda_c_pw_mod_(rho)

    kf = (9 / 4 * np.pi) ** (1 / 3) / rs
    ks = np.sqrt(4 * kf / np.pi)
    divt = 2 * ks * rho
    t = norm_dn / divt
    expec = np.exp(-ec / gamma)
    A = beta / (gamma * (expec - 1))
    t2 = t ** 2
    At2 = A * t2
    A2t4 = At2 ** 2
    divsum = 1 + At2 + A2t4
    div = (1 + At2) / divsum
    nolog = 1 + beta / gamma * t2 * div
    gec = gamma * np.log(nolog)

    factor = A2t4 * (2 + At2) / divsum ** 2
    dgec = beta * t2 / nolog * (-7 / 3 * div - factor * (A * expec * (vc - ec) / beta - 7 / 3))
    gvc = gec + dgec

    vsigmac = beta / (divt * ks) * (div - factor) / nolog

    ec += gec
    vc += gvc
    vsigma = 0.5 * vsigmac

    return ec, vc, vsigma


@njit(cache=True, fastmath=True, error_model="numpy", nogil=True, inline='always')
def gga_c_pbe_sol(rho, sigma):
    """
    PBEsol correlation with NaN handling.
    Corresponds to GGA_C_PBE_SOL with ID 133 in LibXC.
    Reference: Phys. Rev. Lett. 100, 136406 (2008).
    """
    ec, vc, vsigma = gga_c_pbe_sol_(rho, sigma)
    vsigma[np.isnan(vsigma)] = 0
    vc[np.isnan(vc)] = 0
    ec[np.isnan(ec)] = 0
    return ec, vc, vsigma


@fuse(kernel_name='gga_c_pbe_sol_cupy_')
def gga_c_pbe_sol_cupy_(rho, sigma):
    """
    CuPy version of PBEsol correlation.
    Corresponds to GGA_C_PBE_SOL with ID 133 in LibXC.
    Reference: Phys. Rev. Lett. 100, 136406 (2008).
    """
    rho = cp.maximum(rho, 1e-12)

    # cupy.fuse infers the type of bare Python scalars inside the kernel and may
    # evaluate these constants in single precision; np.float64 pins them (see
    # the CPU implementation above, which is the reference).
    beta = np.float64(0.046)
    gamma = np.float64((1 - np.log(2.0)) / np.pi**2)

    pi34 = np.float64((3 / (4 * np.pi)) ** (1 / 3))
    rs = pi34 * rho ** (-1 / 3)
    norm_dn = cp.sqrt(sigma)
    ec, vc = lda_c_pw_mod_cupy_(rho)

    kf = np.float64((9 / 4 * np.pi) ** (1 / 3)) / rs
    ks = cp.sqrt(4 * kf / np.float64(np.pi))
    divt = 2 * ks * rho
    t = norm_dn / divt
    expec = cp.exp(-ec / gamma)
    A = beta / (gamma * (expec - 1))
    t2 = t ** 2
    At2 = A * t2
    A2t4 = At2 ** 2
    divsum = 1 + At2 + A2t4
    div = (1 + At2) / divsum
    nolog = 1 + beta / gamma * t2 * div
    gec = gamma * cp.log(nolog)

    factor = A2t4 * (2 + At2) / divsum ** 2
    dgec = beta * t2 / nolog * (-7 / 3 * div - factor * (A * expec * (vc - ec) / beta - 7 / 3))
    gvc = gec + dgec

    vsigmac = beta / (divt * ks) * (div - factor) / nolog

    ec += gec
    vc += gvc
    vsigma = 0.5 * vsigmac

    return ec, vc, vsigma


def gga_c_pbe_sol_cupy(rho, sigma):
    """
    CuPy version of PBEsol correlation with NaN handling.
    Corresponds to GGA_C_PBE_SOL with ID 133 in LibXC.
    """
    ec, vc, vsigma = gga_c_pbe_sol_cupy_(rho, sigma)
    vsigma[cp.isnan(vsigma)] = 0
    vc[cp.isnan(vc)] = 0
    ec[cp.isnan(ec)] = 0
    return ec, vc, vsigma