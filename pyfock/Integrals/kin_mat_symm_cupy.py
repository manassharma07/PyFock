...
def kin_mat_symm_cupy(basis, slice=None, cp_stream=None):
    ...
    kin_mat_symm_internal_cuda[blocks_per_grid, (thread_x, thread_y), nb_stream](bfs_coords[0], bfs_contr_prim_norms[0], bfs_lmn[0], bfs_nprim[0], bfs_coeffs, bfs_prim_norms, bfs_expnts, a, b, c, d, lower_tri, upper_tri, both_tri_symm, both_tri_nonsymm, T)
    if both_tri_symm:
        symmetrize[blocks_per_grid, (thread_x, thread_y), nb_stream](a,b,c,d,T)
...
def kin_mat_symm_internal_cuda(bfs_coords, bfs_contr_prim_norms, bfs_lmn, bfs_nprim, bfs_coeffs, bfs_prim_norms, bfs_expnts, start_row, end_row, start_col, end_col, lower_tri, upper_tri, both_tri_symm, both_tri_nonsymm, out):
        ...
        if lower_tri or upper_tri or (both_tri_symm and j<=i) or both_tri_nonsymm:
            ...
            for ik in range(bfs_nprim[i]):
                for jk in range(bfs_nprim[j]):
                    ...
                    result_sum += result
            out[i - start_row, j - start_col] = result_sum
