...
from .rys_helpers import Roots, Recur_new, Shift_3c2e
...
def _build_ab_primitive_pairs(
    ...
    max_prim_pairs = bfs_expnts.shape[1] * bfs_expnts.shape[1]
    n_ab = nshells * (nshells + 1) // 2

    ab_nprim_pair = np.zeros(n_ab, dtype=np.int32)
    ab_iprim_a = np.zeros((n_ab, max_prim_pairs), dtype=np.int32)
    ab_iprim_b = np.zeros((n_ab, max_prim_pairs), dtype=np.int32)
    ab_alpha = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_beta = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_alpha_beta = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_gamma_p = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_px = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_py = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_pz = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_exp_x = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_exp_y = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)
    ab_exp_z = np.zeros((n_ab, max_prim_pairs), dtype=np.float64)

    ab_idx = 0
    for ish in range(nshells):
        ibf_a0 = shell_bfs_offset[ish]
        ax0, ay0, az0 = bfs_coords[ibf_a0]
        for jsh in range(ish + 1):
            ibf_b0 = shell_bfs_offset[jsh]
            bx0, by0, bz0 = bfs_coords[ibf_b0]
            xij0 = ax0 - bx0
            xij1 = ay0 - by0
            xij2 = az0 - bz0
            ...
            count = 0
            for iprim_a in range(bfs_nprim[ibf_a0]):
                alpha = bfs_expnts[ibf_a0, iprim_a]
                for iprim_b in range(bfs_nprim[ibf_b0]):
                    beta = bfs_expnts[ibf_b0, iprim_b]
                    gamma_p = alpha + beta
                    alpha_beta = alpha * beta
                    inv_gamma_p = 1.0 / gamma_p
                    screen_ab = np.exp(-alpha_beta * inv_gamma_p * ijsq)
                    if screen_ab < 1.0e-8:
                        continue

                    ab_iprim_a[ab_idx, count] = iprim_a
                    ab_iprim_b[ab_idx, count] = iprim_b
                    ab_alpha[ab_idx, count] = alpha
                    ab_beta[ab_idx, count] = beta
                    ab_alpha_beta[ab_idx, count] = alpha_beta
                    ab_gamma_p[ab_idx, count] = gamma_p
                    ab_px[ab_idx, count] = (alpha * ax0 + beta * bx0) * inv_gamma_p
                    ab_py[ab_idx, count] = (alpha * ay0 + beta * by0) * inv_gamma_p
                    ab_pz[ab_idx, count] = (alpha * az0 + beta * bz0) * inv_gamma_p
                    ab_exp_x[ab_idx, count] = np.exp(-alpha_beta * inv_gamma_p * xij0 * xij0)
                    ab_exp_y[ab_idx, count] = np.exp(-alpha_beta * inv_gamma_p * xij1 * xij1)
                    ab_exp_z[ab_idx, count] = np.exp(-alpha_beta * inv_gamma_p * xij2 * xij2)
                    count += 1

            ab_nprim_pair[ab_idx] = count
            ab_idx += 1
...
def rys_3c2e_symm_test2(
    ...
    # cache = getattr(basis, "_pyfock_rys_3c2e_cache", {})
    # cache_key = (id(auxbasis), tuple(slice))
    # exact_key = (cache_key, "exact")
    # if exact_key in cache:
    #     cached = cache[exact_key]()
    #     if cached is not None:
    #         return cached
    # screened_key = (cache_key, bool(schwarz), float(threshold_schwarz))
    # if screened_key in cache:
    #     cached = cache[screened_key]()
    #     if cached is not None:
    #         return cached
    ...
    ints3c2e = rys_3c2e_symm_test_internal(
        basis.bfs_nao,
        auxbasis.bfs_nao,
        nshells,
        nshells_aux,
    ...
    # cache = getattr(basis, "_pyfock_rys_3c2e_cache", {})
    # cached_ints3c2e = weakref.ref(ints3c2e)
    # if not schwarz:
    #     cache[(cache_key, "exact")] = cached_ints3c2e
    # cache[(cache_key, bool(schwarz), float(threshold_schwarz))] = cached_ints3c2e
    # basis._pyfock_rys_3c2e_cache = cache
    return ints3c2e
...
def rys_3c2e_symm_test_internal(
    ...
    for task_idx in prange(n_tasks):
        ...
        xij1 = ay0 - by0
        xij2 = az0 - bz0
        nprim_pair = ab_nprim_pair[ab_idx]
        if nprim_pair == 0:
            continue
        ...
        for iprim_pair in range(nprim_pair):
            iprim_a = ab_iprim_a[ab_idx, iprim_pair]
            iprim_b = ab_iprim_b[ab_idx, iprim_pair]
            alpha = ab_alpha[ab_idx, iprim_pair]
            beta = ab_beta[ab_idx, iprim_pair]
            alpha_beta = ab_alpha_beta[ab_idx, iprim_pair]
            gamma_p = ab_gamma_p[ab_idx, iprim_pair]
            px = ab_px[ab_idx, iprim_pair]
            py = ab_py[ab_idx, iprim_pair]
            pz = ab_pz[ab_idx, iprim_pair]
            exp_x = ab_exp_x[ab_idx, iprim_pair]
            exp_y = ab_exp_y[ab_idx, iprim_pair]
            exp_z = ab_exp_z[ab_idx, iprim_pair]
            ...
            for iprim_c in range(nprim_c):
                ...
                g00_scale = pi / gamma_pq_sqrt
                g00_x = g00_scale * exp_x
                g00_y = g00_scale * exp_y
                g00_z = g00_scale * exp_z
...
@njit(parallel=True, cache=True, fastmath=True, nogil=True, error_model="numpy")
def rys_3c2e_symm_test_internal_ab_pairs(
    nbf,
    naux,
    nshells,
    nshells_aux,
    ...
    for ab_idx in prange(n_ab):
        ...
        shell_bound = 1.0
        if schwarz:
            shell_bound = shell_pair_bound[ish, jsh]
            if shell_bound < threshold_schwarz:
                continue
        ...
        for ksh in range(nshells_aux):
            ...
            if schwarz:
                if shell_bound * aux_shell_bound[ksh] < threshold_schwarz:
                    continue
            ...
            for iprim_c in range(nprim_c):
                ...
                for iroot in range(nroots):
                    ...
                    for ia in range(nbf_a):
                        for ib in range(nbf_b):
                            iab = ia * nbf_b + ib
                            ab_coeff = ab_pair_coeff[ab_idx, iprim_pair, iab]
                            for ic in range(nbf_c):
                                ibf_c = bf_c_start + ic
                                shell_block[ia, ib, ic] += (
                                    ab_coeff
