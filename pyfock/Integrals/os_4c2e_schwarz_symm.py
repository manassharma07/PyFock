...
def os_4c2e_schwarz_symm(basis, threshold_schwarz=1e-9):
    """
    ...
    ints4c2e = os_4c2e_symm_internal(
        ...
        bfs_coeffs, bfs_expnts, bfs_prim_norms, sqrt_ints4c2e_diag, threshold_schwarz
    )
...
def os_4c2e_symm_internal(nbf, nshells, shell_L, shell_centers, shell_bfs_offset,
                          ...
                          bfs_nprim, bfs_shell_index, bfs_coeffs, bfs_expnts,
                          bfs_prim_norms, sqrt_ints4c2e_diag, threshold_schwarz):
    ...
    # Precompute Schwarz bound for each shell-pair index
    ab_schwarz = np.zeros(n_ab, dtype=np.float64)
    ...
    for a in range(nshells):
        for b in range(a + 1):
            ...
            ab_schwarz[idx] = schwarz_shell_pair[a, b]
            idx += 1
    ...
    for ab_idx in prange(n_ab):
        ...
        # Schwarz bound for this bra shell pair
        schwarz_ab = ab_schwarz[ab_idx]
    ...
    for ix in range(dx + 1):
        ...
        for iy in range(dy + 1):
            ...
            for iz in range(dz + 1):
                ...
                for px in range(bx + 1):
                    binom_x_bra = comb(bx, px)
                    pow_x_bra = X_AB[0] ** (bx - px)
                    ax_final = ax + px

                    for py in range(by + 1):
                        binom_y_bra = comb(by, py)
                        pow_y_bra = X_AB[1] ** (by - py)
                        ay_final = ay + py

                        for pz in range(bz + 1):
                            binom_z_bra = comb(bz, pz)
