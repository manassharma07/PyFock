...
def rys_3c2e_symm_cupy_fp32(basis, auxbasis, slice=None, schwarz=True, schwarz_threshold=1e-9, cp_stream=None):
    ...
    aux_bfs_coords = cp.array([auxbasis.bfs_coords], dtype=cp.float32)
    aux_bfs_contr_prim_norms = cp.array([auxbasis.bfs_contr_prim_norms], dtype=cp.float32)
    aux_bfs_lmn = cp.array([auxbasis.bfs_lmn])
    aux_bfs_nprim = cp.array([auxbasis.bfs_nprim], dtype=cp.float32)
    ...
    bfs_coeffs = cp.zeros([basis.bfs_nao, maxnprim], dtype=cp.float32)
    bfs_expnts = cp.zeros([basis.bfs_nao, maxnprim], dtype=cp.float32)
    bfs_prim_norms = cp.zeros([basis.bfs_nao, maxnprim], dtype=cp.float32)
    ...
    aux_bfs_coeffs = cp.zeros([auxbasis.bfs_nao, maxnprimaux], dtype=cp.float32)
    aux_bfs_expnts = cp.zeros([auxbasis.bfs_nao, maxnprimaux], dtype=cp.float32)
    aux_bfs_prim_norms = cp.zeros([auxbasis.bfs_nao, maxnprimaux], dtype=cp.float32)
    ...
    DATA_X_cuda = cp.asarray(DATA_X, dtype=cp.float32)
    DATA_W_cuda = cp.asarray(DATA_W, dtype=cp.float32)
    ...
    threeC2E = cp.zeros(matrix_shape, dtype=cp.float32)
...
def rys_3c2e_symm_internal_cuda(bfs_coords, bfs_contr_prim_norms, bfs_lmn, bfs_nprim, bfs_coeffs, bfs_prim_norms, bfs_expnts, aux_bfs_coords, aux_bfs_contr_prim_norms, aux_bfs_lmn, aux_bfs_nprim, aux_bfs_coeffs, aux_bfs_prim_norms, aux_bfs_expnts, indx_startA, indx_endA, indx_startB, indx_endB, indx_startC, indx_endC, tri_symm, no_symm, DATA_X, DATA_W, schwarz, sqrt_ints4c2e_diag, sqrt_diag_ints2c2e, schwarz_threshold, out):
    ...
    L = cuda.local.array((3), numba.float32)
    L[0] = 0.0
    L[1] = 0.0
    L[2] = 0.0
    ...
    if i>=indx_startA and i<indx_endA and j>=indx_startB and j<indx_endB:  
        if (tri_symm and j<=i) or no_symm:
            ...
            IJ = cuda.local.array((3), numba.float32)
            P = cuda.local.array((3), numba.float32)
            PQ = cuda.local.array((3), numba.float32)
            I = bfs_coords[i]
            ...
            roots = cuda.local.array((10), numba.float32) # Good for upto i shells; i orbitals have an angular momentum of 6;
            weights = cuda.local.array((10), numba.float32) # Good for upto i shells; i orbitals have an angular momentum of 6;
            G = cuda.local.array((13, 13), numba.float32) # Good for upto i shells; i orbitals have an angular momentum of 6;
                    ...
                    for ik in range(nprimi):   
                        ...
                        for jk in range(nprimj):
                            ...
                            for kk in range(nprimk):
                                ...
                                val += tempcoeff5*coulomb_rys_fp32(roots,weights,G,PQsq, rho, norder,n,m,la,lb,lc,ld,ma,mb,mc,md,na,nb,nc,nd,alphaik, alphajk, alphakk, alphalk,I,J,K,L)
