...
def eval_xc_2(basis, dmat, weights, coords, funcid=[1,7], use_libxc=False, spin=0, ncores=2, blocksize=5000, list_nonzero_indices=None, count_nonzero_indices=None, list_ao_values=None, list_ao_grad_values=None, debug=False, print_nelec=True):
    """
    ...
    In algorithm 2, the XC term is evaluated by parallelizing over batches.
    ...
    basis : Basis
        ...
        angular momentum, normalization, etc.
    ...
    ncores : int, optional
        Number of threads/cores for parallel execution. Default is 2.

    blocksize : int, optional
        Number of grid points to process per block. Default is 5000.

    list_nonzero_indices : list of np.ndarray, optional
        Precomputed list of nonzero AO indices per block for sparse evaluation.
    ...
    debug : bool, optional
        If True, print detailed timing and diagnostic info.
    ...
    for iblock in block_indices:
        efunc += output[indx_block_output][0]
        if list_nonzero_indices is not None:
            non_zero_indices = list_nonzero_indices[iblock][0:count_nonzero_indices[iblock]]
            v[np.ix_(non_zero_indices, non_zero_indices)] += output[indx_block_output][1]
            # slice = np.ix_(non_zero_indices, non_zero_indices)
            # v[slice] = numexpr.evaluate("(v_ + output_)", {'v_':v[slice], 'output_':output[indx_block_output][1]})
        else:
            v += output[indx_block_output][1]
        nelec += output[indx_block_output][2]
        if debug:
            timings['durationLibxc'] += output[indx_block_output][3]['durationLibxc']
            timings['durationE'] += output[indx_block_output][3]['durationE']
            timings['durationF'] += output[indx_block_output][3]['durationF']
            timings['durationZ'] += output[indx_block_output][3]['durationZ']
            timings['durationV'] += output[indx_block_output][3]['durationV']
            timings['durationRho'] += output[indx_block_output][3]['durationRho']
            timings['durationAO'] += output[indx_block_output][3]['durationAO']
        indx_block_output += 1 
        # v = numexpr.evaluate('(v + output[iblock][1])')

    # print('here1')
    # for i in range(1000):
    #     a = dmat[np.ix_(list_nonzero_indices[60][0:count_nonzero_indices[60]], list_nonzero_indices[60][0:count_nonzero_indices[60]])]
    # print('here2')
...
def block_dens_func(weights_block, coords_block, dmat, funcid, use_libxc, bfs_data_as_np_arrays, non_zero_indices=None, ao_values=None, ao_grad_values=None, funcx=None, funcc=None, x_family_code=None, c_family_code=None, xc_family_dict=None, numexpr_expr=None, debug=False):
    ...
    if non_zero_indices is not None:
        ...
        # rho_block = contract('jm,mj->m', Fjm, ao_value_block)

        Fmj = ao_value_block @ dmat # This intermediate helps in accelerating grad rho
        rho_block = contract('mj,mj->m', Fmj, ao_value_block)
        # rho_block = np.sum(Fmj * ao_value_block, axis=1)
    ...
    if xc_family_dict[x_family_code]!='LDA' or xc_family_dict[c_family_code]!='LDA':
        ...
        Fx = numexpr_expr['expr_Fx'](Ftemp, rho_grad_block_x)
        Fy = numexpr_expr['expr_Fy'](Ftemp, rho_grad_block_y)
        Fz = numexpr_expr['expr_Fz'](Ftemp, rho_grad_block_z)
    ...
    if xc_family_dict[x_family_code]!='LDA' or xc_family_dict[c_family_code]!='LDA':
        ...
        # z += numexpr_expr['expr_z_grad'](Fx, ao_value_gradx_block_T, Fy, ao_value_grady_block_T, Fz, ao_value_gradz_block_T)
