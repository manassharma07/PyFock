...
def kin_mat_grad_r_symm(basis, slice=None, wrt_atoms=False):
    ...
    dT_r = kin_mat_symm_grad_r_internal_new(bfs_atoms[0], bfs_coords[0], bfs_contr_prim_norms[0], bfs_lmn[0], bfs_nprim[0], bfs_coeffs, bfs_prim_norms, bfs_expnts, a, b, c, d)
    
    if wrt_atoms:
        return basis_atom_gradient_from_r(dT_r, basis, slice)
    return dT_r 

@njit(parallel=True, cache=True)
def kin_mat_symm_grad_r_internal_new(bfs_atoms, bfs_coords, bfs_contr_prim_norms, bfs_lmn, bfs_nprim, bfs_coeffs, bfs_prim_norms, bfs_expnts, start_row, end_row, start_col, end_col):
                ...
                for ik in range(bfs_nprim[i]):
                    for jk in range(bfs_nprim[j]):
                        ...
                        for dir in range(3):
                            ...
                            if dir==0: #x
                                lfactor = lmni[0]
                                Sx_1 = calcS(lmni[0]-1,lmnj[0],gamma,PI[0],PJ[0])
                                Sy_1 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_1 = calcS(lmni[2],lmnj[2],gamma,PI[2],PJ[2])
                                overlap1 = Sx_1*Sy_1*Sz_1

                                Sx_2 = calcS(lmni[0]-1,lmnj[0]+2,gamma,PI[0],PJ[0])
                                overlap2 = Sx_2*Sy_1*Sz_1

                                Sx_3 = Sx_1#calcS(lmni[0]-1,lmnj[0],gamma,PI[0],PJ[0])
                                Sy_2 = calcS(lmni[1],lmnj[1]+2,gamma,PI[1],PJ[1])
                                overlap3 = Sx_3*Sy_2*Sz_1

                                Sy_3 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_2 = calcS(lmni[2],lmnj[2]+2,gamma,PI[2],PJ[2])
                                overlap4 = Sx_3*Sy_3*Sz_2

                                Sx_4 = calcS(lmni[0]-1,lmnj[0]-2,gamma,PI[0],PJ[0])
                                Sz_3 = calcS(lmni[2],lmnj[2],gamma,PI[2],PJ[2])
                                overlap5 = Sx_4*Sy_3*Sz_3

                                Sx_5 = calcS(lmni[0]-1,lmnj[0],gamma,PI[0],PJ[0])
                                Sy_4 = calcS(lmni[1],lmnj[1]-2,gamma,PI[1],PJ[1])
                                overlap6 = Sx_5*Sy_4*Sz_3

                                Sy_5 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_4 = calcS(lmni[2],lmnj[2]-2,gamma,PI[2],PJ[2])
                                overlap7 = Sx_5*Sy_5*Sz_4
                                ...
                                tempA = -lfactor*temp*(part1 - part2 - 0.5*(part3+part4+part5))*temp_1
                                ...
                                tempB = 2*alphaik*temp*(part1 - part2 - 0.5*(part3+part4+part5))*temp_1
                                result = tempA + tempB 
                            if dir==1: #y
                                lfactor = lmni[1]
                                Sx_1 = calcS(lmni[0],lmnj[0],gamma,PI[0],PJ[0])
                                Sy_1 = calcS(lmni[1]-1,lmnj[1],gamma,PI[1],PJ[1])
                                Sz_1 = calcS(lmni[2],lmnj[2],gamma,PI[2],PJ[2])
                                overlap1 = Sx_1*Sy_1*Sz_1

                                Sx_2 = calcS(lmni[0],lmnj[0]+2,gamma,PI[0],PJ[0])
                                overlap2 = Sx_2*Sy_1*Sz_1

                                Sx_3 = Sx_1#calcS(lmni[0],lmnj[0],gamma,PI[0],PJ[0])
                                Sy_2 = calcS(lmni[1]-1,lmnj[1]+2,gamma,PI[1],PJ[1])
                                overlap3 = Sx_3*Sy_2*Sz_1

                                Sy_3 = calcS(lmni[1]-1,lmnj[1],gamma,PI[1],PJ[1])
                                Sz_2 = calcS(lmni[2],lmnj[2]+2,gamma,PI[2],PJ[2])
                                overlap4 = Sx_3*Sy_3*Sz_2

                                Sx_4 = calcS(lmni[0],lmnj[0]-2,gamma,PI[0],PJ[0])
                                Sz_3 = calcS(lmni[2],lmnj[2],gamma,PI[2],PJ[2])
                                overlap5 = Sx_4*Sy_3*Sz_3

                                Sx_5 = calcS(lmni[0],lmnj[0],gamma,PI[0],PJ[0])
                                Sy_4 = calcS(lmni[1]-1,lmnj[1]-2,gamma,PI[1],PJ[1])
                                overlap6 = Sx_5*Sy_4*Sz_3

                                Sy_5 = calcS(lmni[1]-1,lmnj[1],gamma,PI[1],PJ[1])
                                Sz_4 = calcS(lmni[2],lmnj[2]-2,gamma,PI[2],PJ[2])
                                overlap7 = Sx_5*Sy_5*Sz_4
                                ...
                                # Sx_3 = calcS(lmni[0],lmnj[0],gamma,PI[0],PJ[0])
                                Sy_2 = calcS(lmni[1]+1,lmnj[1]+2,gamma,PI[1],PJ[1])
                                overlap3 = Sx_3*Sy_2*Sz_1
                            ...
                            if dir==2: #z
                                ...
                                Sx_1 = calcS(lmni[0],lmnj[0],gamma,PI[0],PJ[0])
                                Sy_1 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_1 = calcS(lmni[2]-1,lmnj[2],gamma,PI[2],PJ[2])
                                overlap1 = Sx_1*Sy_1*Sz_1
                                ...
                                Sy_3 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_2 = calcS(lmni[2]-1,lmnj[2]+2,gamma,PI[2],PJ[2])
                                overlap4 = Sx_3*Sy_3*Sz_2

                                Sx_4 = calcS(lmni[0],lmnj[0]-2,gamma,PI[0],PJ[0])
                                Sz_3 = calcS(lmni[2]-1,lmnj[2],gamma,PI[2],PJ[2])
                                overlap5 = Sx_4*Sy_3*Sz_3
                                ...
                                Sy_5 = calcS(lmni[1],lmnj[1],gamma,PI[1],PJ[1])
                                Sz_4 = calcS(lmni[2]-1,lmnj[2]-2,gamma,PI[2],PJ[2])
                                overlap7 = Sx_5*Sy_5*Sz_4
                            ...
                            result_sum[dir] += result
                            dT[dir, i - start_row, j - start_col] = result_sum[dir]
                            if both_tri_symm:
                                dT[dir, j - start_col, i - start_row] = -result_sum[dir]
